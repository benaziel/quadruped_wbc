...
which enters the QP as an equality constraint (alongside the friction cone inequality constraints and actuator limit box constraints). The QP has fixed structure: cone and swing-force rows are always present and gait transitions only toggle bounds, so OSQP is set up once and updated in place through precomputed CSC index maps, warm-starting from the previous solution at every control step. A warm-started solve runs in ~43 µs mean; the full controller step including Python-side assembly is ~0.51 ms. 
